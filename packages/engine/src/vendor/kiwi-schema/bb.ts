...
export class ByteBuffer {
  ...
  readByte(): number {
    if (this._index >= this.length) throw new Error('Read past end of buffer');
  ...
  readByteArray(): Uint8Array {
    ...
    if (start + length > this.length) throw new Error('Read past end of buffer');
  ...
  readVarUint(): number {
    ...
    let i = this._index;
    if (i >= len) throw new Error('Read past end of buffer');
    let b = data[i++];
    let value = b & 127;
    if (b < 128) {
      this._index = i;
